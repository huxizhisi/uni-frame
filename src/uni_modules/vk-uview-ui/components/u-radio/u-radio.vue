<template>
    <view class="u-radio" :style="[radioStyle]">
        <view v-if="!elSlot" class="u-radio__icon-wrap" @tap="toggle" :class="[iconClass]" :style="[iconStyle]">
            <u-icon class="u-radio__icon-wrap__icon" name="checkbox-mark" :size="elIconSize" :color="iconColor" />
        </view>
        <template v-if="elSlot">
          <slot :name="elSlotProp"></slot>
        </template>
        <view class="u-radio__label" @tap="onClickLabel" :class="[iconClass]" :style="{ fontSize: $u.addUnit(labelSize) }">
            <slot />
        </view>
    </view>
</template>
...
export default {
  ...
  computed: {
    // 盒子的宽度大小
    elBoxWidth() {
      return !!this.boxWidth ? this.boxWidth : 'auto'
    },
    // 是否使用自定义插槽
    elSlot() {
      return !!this.isSlot
    ...
    // 自定义插槽名字
    elSlotProp() {
      return this.isSlot ? this.slotName : ''
    },
    ...
    iconClass() {
      let classes = [];
      if (this.elSlot) {
        if (this.name === this.parentData.value) classes.push("u-radio__icon-wrap-slot--checked");
      } else {
    ...
    radioStyle() {
      ...
      if (this.parentData.width) {
        style.width = this.$u.addUnit(this.elBoxWidth);
        // #ifdef MP
