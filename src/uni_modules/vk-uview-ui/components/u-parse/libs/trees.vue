        ...
        <block v-for="(n, i) in nodes" v-bind:key="i">
            ...
            >
                <view
                    ...
                    :style="
                        (tbody.attrs.style || '') +
                            (tbody.name[0] == 't' ? ';display:table-' + (tbody.name == 'tr' ? 'row' : 'row-group') : '')
                    "
                >
                    <view
                        ...
                        :style="
                            (tr.attrs.style || '') + (tr.name[0] == 't' ? ';display:table-' + (tr.name == 'tr' ? 'row' : 'cell') : '')
                        "
                    >
...
export default {
  ...
  methods: {
            ...
            setTimeout(() => {
              this.observer.observe("._img", (res) => {
                if (res.intersectionRatio) {
                  for (let j = this.nodes.length; j--; ) if (this.nodes[j].name == "img") this.$set(this.ctrl, j, 1);
    ...
    play(e) {
      let contexts = this.top.videoContexts;
      if (contexts.length > 1 && this.top.autopause)
        for (let i = contexts.length; i--; ) if (contexts[i].id != e.currentTarget.dataset.id) contexts[i].pause();
    },
    imgtap(e) {
    ...
    error(e) {
      ...
      if (source == "video" || source == "audio") {
        // 加载其他 source
        let index = this.ctrl[i] ? this.ctrl[i].i + 1 : 1;
