...
let fs = uni.getFileSystemManager ? uni.getFileSystemManager() : null;
// #endif
let dom;
// 计算 cache 的 key
function hash(str) {
...
let { windowWidth, platform } = uni.getSystemInfoSync();
// #endif
// #ifdef APP-PLUS-NVUE
...
export default {
  ...
  created() {
    ...
    this.imgList.each = function (f) {
      for (let i = 0, len = this.length; i < len; i++) this.setItem(i, f(this[i], i, this));
    };
  ...
  methods: {
    ...
    setContent(html, append) {
      ...
      let _ts = this;
      // 获取标题
      let title = this.rtf.getElementsByTagName("title");
      ...
      let imgs = this.rtf.getElementsByTagName("img");
      ...
      // 链接处理
      let links = this.rtf.getElementsByTagName("a");
      ...
      // 视频处理
      let videos = this.rtf.getElementsByTagName("video");
      ...
      // 音频处理
      let audios = this.rtf.getElementsByTagName("audio");
      ...
      // 表格处理
      if (this.autoscroll) {
        let tables = this.rtf.getElementsByTagName("table");
    ...
    navigateTo(obj) {
      ...
      selector.exec((res) => {
        ...
        let scrollTop = res[1].scrollTop + res[0].top - (res[2] ? res[2].top : 0) + (obj.offset || 0);
    ...
    getVideoContext(id) {
      ...
      if (!id) return this.videoContexts;
        ...
        for (let i = this.videoContexts.length; i--; ) if (this.videoContexts[i].id == id) return this.videoContexts[i];
      // #endif
    },
    // #ifdef H5 || APP-PLUS-NVUE || MP-360
    _handleHtml(html, append) {
      if (!append) {
        ...
        let style = "<style>@keyframes _show{0%{opacity:0}100%{opacity:1}}img{max-width:100%}";
    ...
    _message(e) {
      ...
      switch (d.action) {
        ...
        case "getImgList":
          this.imgList.length = 0;
          for (let i = d.imgList.length; i--; ) this.imgList.setItem(i, d.imgList[i]);
          break;
        case "preview":
          var preview = true;
