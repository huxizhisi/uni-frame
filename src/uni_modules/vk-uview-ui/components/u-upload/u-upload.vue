        ...
        >
            ...
            >
                ...
                <image
                    ...
                    mode="widthFix"
                    name="delIcon"
                    src="data:image/svg+xml;base64,"
                    />
            </view>
            <!-- <view
...
export default {
  ...
  methods: {
      ...
      let fileSystem = uni.getFileSystemManager()
        return new Promise((resolve, reject) => {
            fileSystem.getFileInfo({
                filePath: filePath,
                success: (res) => {
                    resolve(res.digest); // 将结果通过resolve传递出去
                },
                fail: (err) => {
                    reject(err); // 如果发生错误，则通过reject传递出去
                }
            });
        });
    },
    ...
    async uploadFile(index = 0) {
      ...
      let digest
      try {
        digest = await this.getFileInfoAsync( this.lists[index].url);
      } catch (err) {
        console.error(err)
      }
      console.log(this.formData, 'md5', digest)
      this.$set(this.formData, 'md5', digest);
      // 检查上传地址
      if (!this.action) {
