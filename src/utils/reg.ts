...
/** 判断是否在预定时间之前 */
function generateDateTime(advanceBookingDays: number, advanceBookingTime: string, dateString: string) {
  let isGreaterThanAdvanceBookingDays = false;
  const currentDate = new Date();
  const selectedDate = new Date(dateString);
  const [hours, minutes] = advanceBookingTime.split(":");
  ...
  const calculateDifference = (date1: Date, date2: Date) => {
    ...
    isGreaterThanAdvanceBookingDays = differenceInDays >= advanceBookingDays;
  };
  calculateDifference(currentDate, selectedDate);
  return isGreaterThanAdvanceBookingDays;
}

/** 定义校验姓名重复方法 */
function validateCnName(data: any, firstName = "first_name", lastName = "last_name") {
  // 检查姓名是否完全相同
  const isIdentical = data[firstName] === data[lastName];
  // 检查姓氏是否包含在名字的左边
  const isPrefixDuplicate = data[firstName].startsWith(data[lastName]);
  // 重复字段
  let duplicatePartStr = "";
  // 全部重复
  if (isIdentical) {
    duplicatePartStr = data[firstName];
  } else if (isPrefixDuplicate) {
    // 提取重复的部分
    const minLength = Math.min(data[firstName].length, data[lastName].length);
    let i = 0;
    while (i < minLength && data[firstName][i] === data[lastName][i]) {
      i++;
    }
    duplicatePartStr = data[firstName].substring(0, i);
  }
  return { isIdentical, isPrefixDuplicate, duplicatePartStr };
}
